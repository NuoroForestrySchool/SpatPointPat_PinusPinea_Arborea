...
ppwm01 <- ppwm %>%
  ...
  mutate(b = ifelse(is.na(b_corr), b, b_corr),
         ...
         #age = ifelse(is.na(age_corr), age, age_corr)) %>%
...
pin_specie = list("PINPIN" , "CEPPAIA")
alberi_specie = list("EUC")
arbusti_specie = list("PHYANG", "PISLEN", "CALVIL", "DAFGNI",  "QUEILE",  "ALYALY",  "CISSAL", "OLEEUR", "RHAALA" , "ERIARB")
gruppi_specie = list(pin_specie, alberi_specie, arbusti_specie)
for(gruppox in gruppi_specie) {
show(gruppox)
...
#    plot(.$ppp[[p]], main=paste("Plot", .$plot[[p]]), which.marks = m)
  plot(Arborea_permanent_plots$ppp[[p]][toupper(Arborea_permanent_plots$ppp[[p]]$marks$..species) %in% gruppox], main=paste("Plot", p), which.marks = m)
  mtext(colnamesm[m])
  at <- seq(0,50,10)
  Axis(side=1, at=at)
  Axis(side=4, at=at)
...
for(gruppox in gruppi_specie) {
  ...
  pltsm_s = pltsm[toupper(pltsm$marks$..species) %in% gruppox]
  
  marks(pltsm_s) <- pltsm_s$marks[3:4]

  show(paste("Specie:",gruppox))
